...
// Constructor for GetIntegerMenuInterface
GetIntegerMenuInterface::GetIntegerMenuInterface(std::string title) : menuTitle(title) {}

// Method to show the menu
IntegerOption GetIntegerMenuInterface::showMenu() {
    ...
    while (chosenOption.isEmpty) { // Loop until an option is chosen
        ...
        switch (character) {
            case '\n': // If Enter is pressed
                try {
                    chosenOption.setValue(std::stoi(optionName)); // Try to convert text to integer
            ...
            case 27: // ESC key
                ...
                break;
            default:
                if (character >= '0' && character <= '9') { // Accept only digits
